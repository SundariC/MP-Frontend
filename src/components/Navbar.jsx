      ...
      <div className="flex items-center gap-8">
        <div className="hidden lg:flex gap-8 text-sm font-bold text-slate-500">
          <span
            ...
            className="hover:text-[#0D9488] cursor-pointer transition-colors"
          >
            Services
          </span>
          <span
          ...
          <span
            onClick={() => handleScroll("client-section")}
            className="hover:text-[#0D9488] cursor-pointer transition-colors"
          >
            For Client
          </span>
          <span
            onClick={() => handleScroll("counselor-section")}
            className="hover:text-[#0D9488] cursor-pointer transition-colors"
          >
            For Counselor
          </span>
          <span
            onClick={() => handleScroll("privacy-section")}
            className="hover:text-[#0D9488] cursor-pointer transition-colors"
          >
            Privacy
          </span>
        ...
        {user ? (
          <div className="relative" ref={dropdownRef}>
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 p-3 z-[110]">
                <button
                  onClick={() => {
                    navigate(
                      user.role === "client"
                ...
                >
                  <LayoutDashboard
                    size={18}
                    className="text-slate-400 group-hover:text-[#0D9488]"
                  />
                  My Dashboard
                </button>
                <button
                  onClick={handleLogout}
                  className="flex items-center gap-3 w-full p-3 hover:bg-red-50 rounded-xl text-sm font-bold text-red-500"
                >
          ...
          <div className="flex gap-6 items-center">
            ...
            {/* Simple Single Button for Signup */}
            <button

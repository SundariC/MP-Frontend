      ...
      <aside className="w-64 bg-white border-r border-slate-100 hidden md:flex flex-col p-6 sticky top-0 h-screen">
        <nav className="space-y-2 flex-grow text-left">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${activeTab === "requests" ? "bg-[#0D9488] text-white shadow-lg" : "text-slate-500 hover:bg-slate-50"}`}
          >
            <Clock size={20} /> Requests
          </button>
          <button
            onClick={() => setActiveTab("active")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${activeTab === "active" ? "bg-[#0D9488] text-white shadow-lg" : "text-slate-500 hover:bg-slate-50"}`}
          >
            <Video size={20} /> Active
          </button>
          <button
            onClick={() => setActiveTab("completed")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${activeTab === "completed" ? "bg-[#0D9488] text-white shadow-lg" : "text-slate-500 hover:bg-slate-50"}`}
          >
            <ClipboardList size={20} /> Records
          </button>
        </nav>
        <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm text-red-500 hover:bg-red-50 mt-auto">
      ...
      <main className="flex-grow p-8">
        <header className="flex justify-between items-center mb-10">
          ...
          <button onClick={() => setShowBellPopup(!showBellPopup)} className="p-3 bg-white border border-slate-100 rounded-2xl shadow-sm text-slate-400 hover:text-teal-600 relative">
            <Bell size={20} />
            {showBellPopup && <div className="absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-xl border p-4 z-50 text-xs font-bold text-slate-500">No new notifications</div>}
          </button>
        </header>
        ...
        <div className="grid lg:grid-cols-3 gap-8 text-left">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm min-h-[500px]">
              <h3 className="font-bold text-slate-800 mb-8 uppercase text-xs tracking-widest flex items-center gap-2">
                ...
                <span className="bg-teal-50 text-teal-600 px-2 py-0.5 rounded-lg">
                  {activeTab === "requests" ? requests.length : activeTab === "active" ? activeSessions.length : completed.length}
                </span>
              </h3>
                ...
                {activeTab === "requests" && requests.map((b) => (
                  <div key={b._id} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center justify-between outline-none">
                    <div className="flex items-center gap-4 text-left">
                      <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center font-black text-teal-600 border border-slate-100 uppercase">{b.counselor?.fullName?.charAt(0)}</div>
                      <div>
                        <p className="font-black text-slate-800 italic">Dr. {b.counselor?.fullName}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">{new Date(b.appointmentDate).toDateString()} • {b.appointmentTime}</p>
                      </div>
                    </div>
                    <span className="bg-orange-50 text-orange-600 text-[9px] font-black px-3 py-1 rounded-full animate-pulse uppercase">Waiting for Approval</span>
                  </div>
                ))}
                  ...
                  <div key={b._id} className="p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-teal-600 shadow-sm"><User size={20} /></div>
                        <div>
                          <p className="font-black text-slate-800 italic text-lg">Dr. {b.counselor?.fullName}</p>
                          <p className="text-[10px] text-slate-400 font-black italic uppercase">Session is Live</p>
                        </div>
                      </div>
                      <button onClick={() => navigate(`/chat/${b._id}`)} className="p-2 bg-white border rounded-xl text-teal-600 hover:bg-teal-50"><MessageCircle size={20} /></button>
                    </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
              <h4 className="font-bold text-slate-800 mb-4 text-sm italic uppercase tracking-widest">Spending</h4>
              <div className="text-center py-6 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                <p className="text-4xl font-black text-slate-900 tracking-tighter">₹{totalSpent}</p>
                <p className="text-[9px] text-slate-400 font-black uppercase mt-1 italic">Total Invested in Wellness</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-black italic text-slate-800 uppercase tracking-tighter text-left">Session Notes</h3>
              <button onClick={() => setShowNoteModal(false)} className="text-slate-400 hover:text-red-500"><XCircle size={24} /></button>
            </div>
            ...
            <button onClick={() => setShowNoteModal(false)} className="w-full mt-6 bg-slate-900 text-white py-4 rounded-2xl font-black italic text-xs uppercase shadow-lg">CLOSE RECORDS</button>
          </div>
        </div>
      )}
    </div>

...
const Signup = () => {
  const [role, setRole] = useState('client'); // Default-ah client
      ...
      <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md border border-slate-100">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
          <button 
            ...
            className={`flex-1 py-2 rounded-lg font-bold transition ${role === 'client' ? 'bg-white shadow text-teal-600' : 'text-slate-500'}`}
          >
            Client
          </button>
          <button 
            onClick={() => setRole('counselor')}
            className={`flex-1 py-2 rounded-lg font-bold transition ${role === 'counselor' ? 'bg-white shadow text-teal-600' : 'text-slate-500'}`}
          >
            Counselor
          </button>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          {role === 'counselor' && (
            <input 
              type="text" placeholder="Specialization (e.g. Career, Mental Health)" required
              className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-teal-500"
              onChange={(e) => setFormData({...formData, specialization: e.target.value})}
            />
          )}

          <button type="submit" className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-lg hover:bg-teal-700 transition">
            Register
          </button>
        </form>

        <p className="text-center mt-6 text-slate-600 text-sm">
          Already have an account? <span onClick={() => navigate('/login')} className="text-teal-600 font-bold cursor-pointer hover:underline">Log in</span>
        </p>
      </div>
    </div>
  );
